...
def check_regional_authenticity(m_id, text):
    ...
    # Exclude known false positives (e.g. Yulong snow mountain in China)
    if "yulong" in text or "lijiang" in text or "yunnan" in text or "china" in text:
        # Wait, if it says "taiwan" it's ok, but if it has "china" or "yunnan" or "yulong" it's wrong for Xueshan
        if not ("taiwan" in text and m_id == "xueshan" and "yulong" not in text):
            return False

    if m_id == "myeongseongsan":
        # Must be Pocheon, Sanjeong, Myeongseong, or South Korea/Korea + mountain/lake
        ...
        # Must be Taebaek, Cheonjedan, Pyeongchang, Gangwon, or South Korea/Korea + mountain/snow
        ...
        # Must be Yushan, Jade Mountain, Paiyun, Tataka, Taiwan, Nantou, Haiduan, Chiayi
        ...
        # Must be Xueshan, Snow Mountain Taiwan, Shei-Pa, 369 Cabin, Syue, Taiwan, Taichung, Miaoli, Nantou, Hehuan
        ...
        # Must be Yangmingshan, Qixingshan, Qingtiangang, Xiaoyoufeng, Taipei, Taiwan
...
print("\n--- Regionally Verified Candidates ---")
